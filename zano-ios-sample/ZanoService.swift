...
class ZanoService {
    ...
    public func initIpPort(ip: String, port: String, working_dir: String, log_level: Int32) throws
        -> SuccessResult
    {

        let result = ZanoWallet.InitIpPort(
            ip: ip, port: port, working_dir: working_dir, log_level: log_level)
    ...
    public static func getAddressInfo(addr: String) throws -> AddressInfo {
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: AddressInfo.self)
        return result
    }
    ...
    public func getConnectivityStatus() throws -> ConnectivityStatus {
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: ConnectivityStatus.self)
        return result
    }

    public static func restore(
        seed: String, walletName: String, password: String, seed_password: String
    ) throws -> WalletResult {

        let jsonStr = ZanoWallet.restore(
            seed: seed, walletName: walletName, password: password, seed_password: seed_password)

        guard let jsonData = jsonStr.data(using: .utf8) else {
            throw ZANOError.conversionFailure(message: "")
        }
        let result = try JSONRPCParser.parseResponse(
            jsonData: jsonData, resultType: WalletResult.self)
    ...
    public func getWalletNames() throws -> GetWalletFileResult {
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: GetWalletFileResult.self)
        return result
    }
    ...
    public func asyncCall(methodName: String, walletId: UInt64, params: String) async throws
        -> JobResult
    {

        let jsonStr = try ZanoWallet.asyncCall(
            methodName: methodName, walletId: walletId, params: params)
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: JobResult.self)
        return result
    }
    ...
    public func asyncGetRestoreInfo(jobId: UInt64) async throws -> WalletSyncStatus {
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: WalletSyncStatus.self)
        return result
    ...
    public func asyncGetBalance(jobId: UInt64) async throws -> BalanceResult {
        ...
        let result = try JSONRPCParser.parseResponse(
            jsonData: jsonData, resultType: BalanceResult.self)
        return result
    ...
    public func asyncTransfer(jobId: UInt64) async throws -> TransferResponse {
        ...
        let result = try JSONRPCParser.parseResponse(
            jsonData: jsonData, resultType: TransferResponse.self)
        return result
    }
    ...
    public func asyncOpen(jobId: UInt64) async throws -> AsyncOpenSuccessResponse {
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: AsyncResult.self)

        debugPrint(
            "try JSONRPCParser.parseJsonResponse(jsonData: jsonData, resultType: AsyncResult.self returns "
        )
        debugPrint("\(result)")
    ...
    public func asyncClose(jobId: UInt64) async throws -> SuccessResult {
        ...
        let result = try JSONRPCParser.parseJsonResponse(
            jsonData: jsonData, resultType: SuccessResult.self)
        return result
    }
