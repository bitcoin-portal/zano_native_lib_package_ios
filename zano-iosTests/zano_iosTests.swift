...
class zano_iosTests: XCTestCase {
    ...
    func testTruncateLog() throws {
        let result = try ZanoWallet.truncateLog().returnCode
        XCTAssertEqual(result, "OK")
    }

    func testInit() throws {
        let result = try ZanoWallet.InitIpPort(ip: ZANOConst.ZANO_TESTNET_IP, port: ZANOConst.ZANO_TESTNET_PORT, working_dir: WorkingDir.dir ?? "", log_level: 0)
        XCTAssertEqual(result.returnCode, "OK")
    }
...
class ParseTests: XCTestCase {
    ...
    func testAsyncResponseSuccess() throws {
        ...
        XCTAssertEqual(syncSuccessResponse.balance.balance, 4990000000000)
